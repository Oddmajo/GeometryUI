    ...
    public PlanarGraphConstructor(Diagram diagram)
    {
        ...
        for (int i = 0; i < num_original_segments - 1; i++)
        {
            for (int j = i + 1; j < num_original_segments; j++)
            {
                ...
                if (_segments.get(i).intersects(_segments.get(j)))
                {
                    Point intersection = _segments.get(i).segmentIntersection(_segments.get(j));
                    if (intersection != null)
                    {
                        if (_segments.get(i).pointLiesBetweenEndpoints(intersection) ||
                                _segments.get(j).pointLiesBetweenEndpoints(intersection))
                        {
                            if (!_points.contains(intersection))
                            {
                                _points.add(intersection);
                                if (_segments.get(i).has(intersection))
                                    _segments.add(new Segment(intersection, _segments.get(i).other(intersection)));
                                if (_segments.get(j).has(intersection))
                                    _segments.add(new Segment(intersection, _segments.get(j).other(intersection)));
                            }
                        }
