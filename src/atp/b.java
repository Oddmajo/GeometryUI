...
class b {
	...
	private final Vector<e> _fldfor;
	...
	public static char TranslationC[] = { '\u03B1', '\u0391', '\u03B2',
		'\u0392', '\u03B3', '\u0393', '\u03B4', '\u0394', '\u03B5',
		'\u0395', '\u03D5', '\u03A6', '\u03C6', '\u03B3', '\u0393',
		'\u03B7', '\u0397', '\u03B9', '\u0399', '\u03BA', '\u039A',
		'\u03BB', '\u039B', '\u03BC', '\u039C', '\u03BD', '\u039D',
	...
	public String TranslationS[] = { "alpha", "Alpha", "beta", "Beta", "gamma",
			"Gamma", "delta", "Delta", "epsilon", "Epsilon", "phi", "Phi",
			"varphi", "gamma", "Gamma", "eta", "Eta", "iota", "I", "kappa",
			"Kappa", "lambda", "Lambda", "mu", "Mu", "nu", "Nu", "omikron",
			"Omikron", "pi", "Pi", "chi", "Chi", "rho", "Rho", "sigma",
			"Sigma", "tau", "Tau", "uu", "Uu", "psi", "Psi", "omega", "Omega",
			"chi", "Chi", "ypsilon", "Ypsilon", "zeta", "Zeta", "theta",
			"Theta", "dollar", "percent", "ampersand", "paragraph",
			"copyright", "trademark", "promille", "half", "third", "quarter",
			"threequarters", "twothirds", "pm", "partial", "deltaop", "infty", };
	...
	public String a(final int i, final int j) {
	...
	public void a(final String s) {
		...
		for (; i < _fldfor.size(); i++) {
			if (((e) _fldfor.elementAt(i)).y == 114) {
				...
				for (; k > 0; k--) {
					if (((e) _fldfor.elementAt(k)).y == 4) {
						j1--;
					} else if (((e) _fldfor.elementAt(k)).y == 3) {
						j1++;
					}
					if (j1 == 1) {
						break;
				...
				for (; i2 < _fldfor.size(); i2++) {
					if (((e) _fldfor.elementAt(i2)).y == 4) {
						j1++;
					} else if (((e) _fldfor.elementAt(i2)).y == 3) {
						j1--;
					}
					if (j1 == 1) {
						break;
			...
			} else if (((e) _fldfor.elementAt(i)).y == 115) {
				int l = i - 1;
				int k1 = 0;
				for (; l > 0; l--) {
					if (((e) _fldfor.elementAt(l)).y == 4) {
						k1--;
					} else if (((e) _fldfor.elementAt(l)).y == 3) {
						k1++;
					}
					if (k1 == 1) {
						break;
		...
		for (int j = 0; j < _fldfor.size() - 2; j++) {
			if (((e) _fldfor.elementAt(j)).y == 13
					&& ((e) _fldfor.elementAt(j + 1)).y == 109) {
				int i1 = j + 2;
				int l1 = 0;
				int j2 = 1;
				for (; i1 < _fldfor.size(); i1++) {
					if (((e) _fldfor.elementAt(i1)).y == 4) {
						l1--;
					} else if (((e) _fldfor.elementAt(i1)).y == 3) {
						l1++;
					}
					if (l1 != 0) {
						continue;
					...
					if (((e) _fldfor.elementAt(i1)).w.equals("[")) {
						j2++;
					} else if (((e) _fldfor.elementAt(i1)).w.equals("]")) {
						j2--;
					}
					if (j2 != 0) {
						continue;
			...
			case 92: // '\\'
				...
				case 122: // 'z'
					...
					if ((" arccos arcsin arctan arg cos cosh cot coth csc def deg dim exp hom ker lg ln lo"
							+ "g sec sin sinh tan tanh ").indexOf(" " + s1
									+ " ") >= 0) {
						return new e(9, s1);
					}
					if (" det gcd inf lim max min Pr sup ".indexOf(" " + s1
							+ " ") >= 0) {
						return new e(24, s1);
					}
					...
					if ((" alpha delta epsilon iota kappa lambda nu omega pi sigma theta tau upsilon varep"
							...
							+ "gleftrightarrow longmapsto ").indexOf(" " + s1
									+ " ") >= 0) {
						return new e(18, s1);
					}
					if ((" beta chi eta gamma mu psi phi rho varrho varsigma varphi xi zeta le leq ge geq "
							...
							+ "inus unlhd unrhd bigcirc ").indexOf(" " + s1
									+ " ") >= 0) {
						return new e(19, s1);
					}
					...
					if (" Uparrow Downarrow Updownarrow ".indexOf(" " + s1
							+ " ") >= 0) {
				...
				default:
					a();
				// System.out.println("Scanner invalid tag: \\" +
