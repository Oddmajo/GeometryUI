...
public class sHotEqn {
	...
	public Dimension getPreferredSize() {
		if ((width == 0) & (height == 0)) {
			final Graphics2D g1 = (Graphics2D) PC.getGraphics();
			if (g1 != null) {
				g1.setFont(d);
				S._mthdo();
	...
	public Dimension getSizeof(final String s1, final Graphics2D g1) {
		if (d != g1.getFont()) {
			d = g1.getFont();
			final int fs = d.getSize();
			b = new Font(d.getFamily(), d.getStyle(), fs * ff1 / 10);

			_fldnull = _fldlong = new Font(d.getFamily(), d.getStyle(), fs
					* ff2 / 10);
		}
		...
		return new Dimension(1 + c1._flddo + 2 * byte0, 1 + c1._fldif + c1.a
				+ 2 * byte0);
	}

	public int getAscent(final String s1, final Graphics2D g1) {
		if (d != g1.getFont()) {
			d = g1.getFont();
			final int fs = d.getSize();
			b = new Font(d.getFamily(), d.getStyle(), fs * ff1 / 10);
	...
	public synchronized int paint(final int cc, final int rr, final Graphics2D g2) {
//                g2.setStroke(new BasicStroke(1.0f));
	...
	private c _mthnew(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1) {
		return _mthtry(i1, j1, flag, g1, k1, true);
	}

	private c _mthtry(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1, final boolean flag1) {
		c c1 = new c();
		final c c2 = new c();
		boolean flag2 = true;
		boolean flag5 = false;
		...
		while (!S._mthnew() && flag2) {
			...
			if (flag) {
				if (A) {
					if (!c && i1 + c2._flddo <= _fldelse
							&& _fldelse <= i1 + c2._flddo + c1._flddo
							&& j1 - c1._fldif <= _fldchar
							&& _fldchar <= j1 + c1.a) {
						m = l = _fldelse;
						a = R = _fldchar;
						c = true;
					...
					if (!c && i1 + c2._flddo <= J
							&& J <= i1 + c2._flddo + c1._flddo
							&& j1 - c1._fldif <= I && I <= j1 + c1.a) {
						m = l = J;
						a = R = I;
						c = true;
						p = l1;
	...
	private c _mthbyte(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1) {
		c c1 = new c();
		int l1 = 0;
		...
		if (flag) {
			...
			if (s1.equals(".") | s1.equals("..")) {
				...
				g1.drawString(s1, i1 + l2, j1 - (fontmetrics.getAscent() * 2)
						/ 3);
			}
		}
	...
	private c a(final int i1, final int j1, final boolean flag,
			final Graphics2D g1) {
		new c();
		final FontMetrics fontmetrics = g1.getFontMetrics();
	...
	private c _mthif(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1) {
		int l2 = 0;
		...
		if (flag) {
			...
			for (int k4 = 0; k4 < 99; k4++) {
				...
				for (int l4 = 0; l4 < 99; l4++) {
					_mthnew(i1 + l1, (j1 - l2 / 2 - fontmetrics.getDescent())
							+ j2, true, g1, k1);
					l1 += ai[l4];
					if (D.y == 8 || D.y == 4) {
						break;
		...
		return new c(i4 - j3, l2 / 2 + fontmetrics.getDescent(), l2 / 2
				- fontmetrics.getDescent());
	}

	private c _mthgoto(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1) {
		int i2 = 0;
		int j3 = 0;
		final int ai[] = new int[100];
							...
							for (e2 = S._mthint(); e2.y != 4; e2 = S._mthint()) {
								...
								for (int l8 = 0; l8 < stringbuffer1.length(); l8++) {
									...
									case 64: // '@'
										ai3[l5] = 4;
										ai4[l5] = S._mthif();
										final c c3 = _mthtry(i1, j1, false, g1,
												k1, false);
										k3 += c3._flddo;
			...
			for (int k7 = 0; k7 < 99; k7++) {
				...
				for (int k8 = 0; k8 < 99; k8++) {
					while (ai3[i8] == 4) {
						...
						S.a(ai4[i8]);
						final c c5 = _mthtry(i1 + l1, (j1 - i3) + k2, true, g1,
								k1, false);
						l1 += c5._flddo;
						S.a(k4);
						i8++;
					...
					case 2: // '\002'
						...
						S.a(l4);
						c7 = _mthnew(i1 + l1 + (ai[k8] - c7._flddo) / 2,
								(j1 - i3) + k2, true, g1, k1);
						i8++;
						break;
					...
					case 3: // '\003'
						...
						S.a(i5);
						c8 = _mthnew((i1 + l1 + ai[k8]) - c8._flddo, (j1 - i3)
								+ k2, true, g1, k1);
						i8++;
						break;
					}
					...
					for (; ai3[i8] == 4; i8++) {
						...
						S.a(ai4[i8]);
						final c c9 = _mthtry(i1 + l1, (j1 - i3) + k2, true, g1,
								k1, false);
						l1 += c9._flddo;
						S.a(j5);
					}
		...
		if (!a(4, "BEGIN 2: end")) {
			...
			return new c(i2 + k3, j3 / 2 + fontmetrics.getDescent(), j3 / 2
					- fontmetrics.getDescent());
		}
	}

	private c _mthelse(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1) {
		c c1 = new c();
		final int l1 = g1.getFont().getSize() / 2;
		c1 = _mthtry(i1 + l1, j1, flag, g1, k1, false);
		if (flag) {
			g1.drawRect(i1 + l1 / 2, j1 - c1._fldif - l1 / 2, c1._flddo + l1,
					c1._fldif + c1.a + l1);
		}
		return new c(c1._flddo + l1 + l1, c1._fldif + l1, c1.a + l1);
	}

	private c _mthfor(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1, final boolean flag1) {
		int l1 = 0;
		new c();
		c c3 = new c();
		...
		if (flag) {
			...
			_mthtry(i1 + (l2 - c3._flddo) / 2, j1 - 2 - c3.a - l1, true, g1,
					k1 + 1, false);
			if (c && k1 < F) {
				F = k1;
			}
			_mthtry(i1 + (l2 - c4._flddo) / 2, (j1 + 1 + c4._fldif) - l1, true,
					g1, k1 + 1, false);
		}
		a(g1, k1);
	...
	private void a(final Graphics2D g1, final int i1, final int j1, final int k1,
			final int l1, final int i2) {
		g1.drawArc(i1 - k1, j1 - k1, 2 * k1, 2 * k1, l1, i2);
	}

	private void a(final Graphics2D g1, final String s1, final int i1,
			final int j1, final int k1, final int l1, final int i2, final int j2) {
		final int k2 = j1 / 2;
		final int l2 = i1 + k2;
		final int i3 = i1 + j1;
	...
	private c _mthnull(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1) {
		int l1 = 0;
		int i2 = 0;
		c c1 = new c();
		...
		if (flag) {
			...
			if (!s2.equals(".")) {
				if (k4 < l2 && !s2.equals("<") && !s2.equals(">")) {
					g1.setFont(font1);
					g1.drawString(s2, i1, j4 - fontmetrics.getDescent()
			...
			if (!s4.equals(".")) {
				if (k4 < l2 && !s4.equals("<") && !s4.equals(">")) {
					...
					g1.drawString(s4, i1 + j3 + l1 + i3, j4
							- fontmetrics.getDescent()
							- fontmetrics.getLeading() / 2);
	...
	private c _mthdo(final int i1, final int j1, final boolean flag,
			final Graphics2D g1) {
		int k1 = 0;
		int l1 = 0;
		int i2 = 0;
	...
	private c _mthtry(final int i1, final int j1, final boolean flag,
		...
		if (flag) {
			g1.drawLine(i1 + c1._flddo / 4, j1 + c1.a,
					i1 + (c1._flddo * 3) / 4, j1 - c1._fldif);
		}
		return c1;
	}

	private c _mthint(final int i1, final int j1, final boolean flag,
			final Graphics2D g1) {
		final FontMetrics fontmetrics = g1.getFontMetrics();
		if (flag) {
		...
		return new c(fontmetrics.stringWidth(D.w) + 2, fontmetrics.getHeight()
				- fontmetrics.getDescent(), fontmetrics.getDescent());
	}
	...
	private c a(final int i1, final int j1, final boolean flag,
		...
		if (flag) {
			...
			final int l4 = S._mthif();
			if (a(10)) {
				_mthint((i1 + l3) - k2 / 2, j1 - l2 - i2 - j2, true, g1, k1,
						false);
			} else {
				S.a(l4);
			}
	...
	private c _mthlong(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1) {
		int l1 = 0;
		c c1 = new c();
		final int i2 = g1.getFont().getSize() / 4;
		...
		if (flag) {
			...
			final int l4 = S._mthif();
			if (S._mthint().y == 11) {
				_mthdo((i1 + l3) - k2 / 2, j1 + l2 + i2 + j2, true, g1, k1,
						false);
			} else {
				S.a(l4);
			}
	...
	private c a(final int i1, final int j1, final boolean flag,
		...
		if (flag) {
			S.a(l1);
			if (flag1) {
				g1.drawLine(i1 + 1, (j1 - i2) + 2, (i1 + c1._flddo) - 1,
						(j1 - i2) + 2);
			} else {
	...
	private c _mthtry(int i1, final int j1, final boolean flag,
			final Graphics2D g1) {
		final FontMetrics fontmetrics = g1.getFontMetrics();
		...
		return new c(l1, fontmetrics.getHeight() - fontmetrics.getDescent(),
				fontmetrics.getDescent());
	}
	...
	private c _mthif(final int i1, final int j1, final boolean flag,
		...
		return new c(fontmetrics.stringWidth(D.w), fontmetrics.getHeight()
				- fontmetrics.getDescent(), fontmetrics.getDescent());
	}
	...
	private c _mthcase(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1) {
		c c1 = new c();
		int l1 = 0;
		final FontMetrics fontmetrics = g1.getFontMetrics();
		...
		if (flag) {
			...
			g1.drawLine(i1 + k4 + j3, (j1 + j4) - 1, (i1 + k4 + i3) - 2,
					(j1 - i4) + 2);
			g1.drawLine((i1 + k4 + i3) - 2, (j1 - i4) + 2, i1 + l3,
					(j1 - i4) + 2);
			if (flag2) {
				S._mthint();
				a(g1, k1 + 1);
				if (i2 >= j3) {
					...
					_mthtry(i1 + 1, j1 - i4 / 2 - k2 - 1, true, g1, k1 + 1,
							true);
				} else {
					_mthtry(i1 + 1 + (j3 - i2), j1 - i4 / 2 - k2 - 1, true, g1,
							k1 + 1, true);
				}
				a(g1, k1);
			}
	...
	private c _mthchar(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, final int k1) {
		c c1 = new c();
		int l1 = 0;
	...
	private Image _mthif(final Graphics2D g1, final int i1) {
	...
	private c _mthif(final int i1, final int j1, final boolean flag,
			final Graphics2D g1, int k1, final boolean flag1) {
		final FontMetrics fontmetrics = g1.getFontMetrics();
	...
	private c _mthint(final int i1, final int j1, final boolean flag,
		...
		if (flag) {
			...
			if (s1.equals("")) {
				...
				g1.drawLine(i1 + (int) ((double) i2 * 0.80000000000000004D), j3
						- k3, l3, j3);
				g1.drawLine(i1 + (int) ((double) i2 * 0.80000000000000004D), j3
						+ k3, l3, j3);
				...
				for (int l4 = 1; l4 < j2; l4++) {
					final int j4 = k4;
					k4 = (int) ((double) k3 * Math
							.sin((4.0840704496667311D * (double) l4)
									/ (double) j2));
	...
	private void a(final Graphics2D g1, final int i1) {
		if (i1 <= 1) {
			g1.setFont(d);
		} else if (i1 == 2) {
			g1.setFont(b);
