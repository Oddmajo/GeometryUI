...
public abstract class GroundedClause
{
    ...
    public void AddComponent(int component) { utilities.Utilities.AddUnique(figureComponents, component); }
    public void AddComponentList(ArrayList<Integer> componentList) { utilities.Utilities.AddUniqueList(figureComponents, componentList); }
    ...
    */
    ...
    public void AddRelationPredecessors(ArrayList<Integer> preds)
    {
        for (int pred : preds)
        {
            if (pred == -1)
            {
                exceptionHandler.ExceptionHandler.throwException(new ConcreteASTException("ERROR: id is -1: " + pred));
            }
            utilities.Utilities.AddUnique(relationPredecessors, pred);
        }
    }
    public void AddGeneralPredecessors(ArrayList<Integer> preds)
    {
        for (int pred : preds)
        {
            ...
            utilities.Utilities.AddUnique(generalPredecessors, pred);
        }
    }
    ...
    {
        return new ArrayList<GroundedClause>(utilities.Utilities.MakeList(this));
    }
